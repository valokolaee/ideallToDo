...
export default memo((task?: ITask) => {
    ...
    return (
        <View style={[defStyle.main]}>
            <SelectDrop
                data={['yes', 'no', 'go']}
                onSelect={(selectedItem, index) => { mrvLogs(selectedItem, index.toString()) }}
                renderButton={(selectedItem, isOpen) => <CText text={'r'} />}

                renderItem={(selectedItem, index, isSelected) => <CText text={'selectedItem'} />}
                search
